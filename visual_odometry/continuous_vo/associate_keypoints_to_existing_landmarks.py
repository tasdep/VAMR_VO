...
def track_and_update(
    ...
    tracked_new = new_keypoints[status == 1]
    tracked_prev = prev_keypoints[status == 1]
    tracked_3D = state.X[:, status.flatten().astype(bool)]
    ...
    inlier_3D = tracked_3D[:, inlier_mask.flatten().astype(bool)]

    if visualize:
        visualize_keypoint_movement_with_status(
            "After running ransac",
            visualizer_img,
            inlier_prev.reshape(-1, 1, 2),
            inlier_new.reshape(-1, 1, 2),
            np.ones((inlier_new.shape[0], 1)),
        )

    # Update keypoints in the state
    # Transform back to 2xN format
...
def visualize_keypoint_movement_with_status(
    ...
    Parameters:
        ...
        prev_keypoints (np.ndarray): The keypoints in the previous image, in the shape returned by OpenCV: (Mx1x2)
        new_keypoints (np.ndarray): The keypoints in the new image, in the shape returned by OpenCV: (Mx1x2)
    ...
    for p1, p2, s in zip(prev_keypoints, new_keypoints, status):
        # Convert to integer tuples and conver to x,y (swap axes)
        p1 = (int(p1[0][1]), int(p1[0][0]))
        p2 = (int(p2[0][1]), int(p2[0][0]))
