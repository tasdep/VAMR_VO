...
def estimating_current_pose(
    ...
    succes, rotation_vector, translation_vector, inliers_index = cv2.solvePnPRansac(
        landmarks, keypoints, K, **params_RANSAC, flags=PnP_flag
    )
    ...
    if succes == True:
        ...
        # Get the outliers:
        outlier_keypoints = np.zeros(
            (number_of_outliers, keypoints.shape[1], keypoints.shape[2])
        )  # dim: (number_of_points-num_of_inliers) x1x2
    ...
    if refine_with_DLT == True:
        ...
        succes_DLT, rotation_vector_DLT, translation_vector_DLT = cv2.solvePnP(
            inlier_landmarks, inlier_keypoints, K, None, flags=cv2.SOLVEPNP_ITERATIVE
        )
    ...
    # make P homogenous
    X_old = np.vstack([X_old, np.ones((1, X_old.shape[1]))])

    # get points in different camera frames
    P_C: np.ndarray = K @ np.c_[R, t] @ X_old
    ...
    state.update_landmarks(X_new, P_new)
    return R, t
