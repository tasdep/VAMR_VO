...
def distribute_keypoints(image, keypoints, grid_size=(10, 10), keypoints_per_cell=10):
    ...
    for keypoint in keypoints:
        ...
        row = int(keypoint.pt[1] // cell_height)
        col = int(keypoint.pt[0] // cell_width)

        # Add the keypoint to the cell's list of keypoints
        cell_index = row * grid_size[1] + col
        distributed_keypoints[cell_index].append(keypoint)

    # Now ensure each cell has the desired number of keypoints
    for i, cell_keypoints in enumerate(distributed_keypoints):
        if len(cell_keypoints) > keypoints_per_cell:
            cell_keypoints = cell_keypoints[: keypoints_per_cell + 1]

    # Flatten the list of lists into a single list of keypoints
    all_keypoints = [kp for sublist in distributed_keypoints for kp in sublist]
...
def get_candidate_keypoints(
    ...
    number_of_features = 2000 if params.DISTRIBUTE_KEYPOINTS else 500
    sift = cv2.SIFT.create(nfeatures=number_of_features)
    equalized_image = cv2.equalizeHist(img_new)
    sift_keypoints = sift.detect(equalized_image)

    # Distribute the keypoints
    if params.DISTRIBUTE_KEYPOINTS:
        sift_keypoints = distribute_keypoints(equalized_image, sift_keypoints)

    # Convert keypoints for further processing if necessary
    keypoints_new = cv2.KeyPoint.convert(sift_keypoints)

    # Really good detector, but not implemented in exercises. Useful for baseline.
    # akaze = cv2.AKAZE.create()
    # akaze_keypoints = akaze.detect(equalized_image)
...
def triangulate_candidates(
    ...
    v_orig = unit_vector_to_pixel_in_world(
        K, old_state.T.T.reshape((-1, 4, 4)), old_state.F
    )
    T_curr_stack = np.tile(
        current_camera_pose.reshape((4, 4)), (old_state.C.shape[1], 1, 1)
    )
...
def unit_vector_to_pixel_in_world(
    ...
    vector_camera_coordinates = direction_camera_coordinates / np.linalg.norm(
        direction_camera_coordinates, axis=0
    )  # shape
    # R = Nx3x3
    ...
    vector_world_coordinates = np.einsum(
        "nik,kn->in", R.transpose((0, 2, 1)), vector_camera_coordinates
    )
